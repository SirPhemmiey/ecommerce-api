...
describe("Product", () => {
  ...
  describe("GET Products with pagination", () => {
    it("Should return an array of products with page and limit", (done) => {
      request(app)
      .get("/product/getProducts?page=3&limit=10")
      .expect("Content-Type", /json/)
      .expect(200)
      .end((err, res) => {
        if (err) return done(err);
  ...
  describe("Search products", () => {
    it("Should return the search result", (done) => {
      request(app)
      .get("/product/searchProducts?search_term=coat")
      .expect("Content-Type", /json/)
      .expect(200)
      .end((err, res) => {
        if (err) return done(err);
